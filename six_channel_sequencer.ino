...
//ロータリーエンコーダ設定
Encoder myEnc(ENCODER_PIN1, ENCODER_PIN2);//ロータリーエンコーダライブラリ用
int oldPosition  = -999;
int newPosition = -999;
...
void loop() {
 ...
 if ( newPosition < oldPosition ) {//左回し
   oldPosition = newPosition;
   enc = enc - 1;
   refresh_display = true;
 }

 else if ( newPosition > oldPosition ) {//右回し
   oldPosition = newPosition;
   enc = enc + 1;
   refresh_display = true;
 }
 ...
 button = 1 - digitalRead(ENCODER_SW_PIN); //input_pullupのため"1-"
 ...
 if ((old_clock_in == 0 && clock_in == 1) || (refresh_display)) {
   OLED_display();
 }
